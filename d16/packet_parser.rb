INPUT = %Q{}
#INPUT = %Q{D2FE28}
DATA = INPUT.chars
           .map{ _1.to_i(16).to_s(2) }
           .map{ '%04d' % _1.to_i }
           .join

$last_position = 0

class Packet
  attr_accessor :version, :type, :value, :packets

  def initialize
    @packets = []
  end
  def has_sub_packages?
    return packets.size > 0
  end
end

def parse_packet(data, packet)
  version = data[0..2].to_i(2)
  type = data[3..5].to_i(2)

  if type == 4
    literal_number = ""
    start_pos = 6
    while true
      group = data[(start_pos)..(start_pos + 4)]
      literal_number += group[1..5]
      start_pos += 5
      break if group[0] == "0"
    end
    literal_number = literal_number.to_i(2)
    puts "Package version: #{version}, type: #{type}, literal: #{literal_number}"
    packet.version = version
    packet.type = type
    packet.value = literal_number
    $last_position = start_pos
  else
    # so this is an operator package
    mode = data[6].to_i(2)
    if mode == 1
      length = data[7..17].to_i(2)
      start_pos = 18
      length.times do
        packet.packets << parse_packet(data[start_pos..], packet)
        start_pos += $last_position
      end
      packet.type = type
      packet.version = version
    else
      # If the length type ID is 0,
      # then the next 15 bits are a number that represents the total length in bits
      # of the sub-packets contained by this packet.
      length = data[7..(7+14)].to_i(2)
      sub_packets_data = data[22..(22+length-1)]

      while sub_packets_data.length > 0 do
        packet.packets << parse_packet(sub_packets_data, packet)
        sub_packets_data = sub_packets_data[$last_position..]
      end
    end
  end
  return packet
end

packet = parse_packet(DATA, Packet.new)
p sum_version(packet)
p extract_value(packet)
puts packet.flatten.inspect

